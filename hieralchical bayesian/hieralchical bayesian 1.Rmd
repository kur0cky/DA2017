...
```{r newRepeaterSpan_weiubll, include=FALSE}
newRepeaterSpan %>% 
  ggplot(aes(x=diff))+
  geom_density(fill="blue", alpha=0.2, colour = "blue")+
...
```{r lognormal_mcmc, include=FALSE}
data.lognormal <- list(x = as.numeric(newRepeaterSpan$mean),
                     N = nrow(newRepeaterSpan))

fit.lognormal <- stan(file = 'model/lognormal.stan', data = data.lognormal, 
            iter = 1000, chains = 4)

fit.lognormal
traceplot(fit.lognormal)
# 収束性問題なさそう
